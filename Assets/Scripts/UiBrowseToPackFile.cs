...
public class UiBrowseToPackFile : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        var success = FileBrowser.AddQuickLink("Users", "/Users/davidbarile/Desktop", null);

        //Debug.Log($"Quick link added successfully: {success}");
    ...
    private void OnFilesSelected(string[] filePaths)
    {
        ...
        //     Debug.Log($"filePaths[{i}] = {filePaths[i]}");
        ...
        if(fileName.Contains("data_"))
        {
            ...
            var start = data.Substring(0, 40);
            var end = data.Substring(data.Length - 40, 40);

            var startEnd = $"{start}\n.\n.\n.\n{end}";

            this.inputField.text = startEnd;
            this.importPackPopup.ImportPackJsonText = data;
        }
        else
        {
            this.importPackPopup.SetOutputTextThenHide("Please select a valid Mage Wars deck text file.");
            this.inputField.text = string.Empty;
            this.importPackPopup.ImportPackJsonText = string.Empty;
        }
    }

    public void HandleBrowseToFileButtonPress()
    {
        StartCoroutine( ShowLoadDialogCoroutine() );
    }
}